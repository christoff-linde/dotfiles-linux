...
case $desktop in
    ...
    if type "xrandr" > /dev/null; then
      for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
        MONITOR=$m polybar --reload mainbar-i3 -c ~/.config/polybar/config.ini.ini &
      done
    else
    polybar --reload mainbar-i3 -c ~/.config/polybar/config.ini.ini &
    fi
    # second polybar at bottom
    # if type "xrandr" > /dev/null; then
    #   for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
    #     MONITOR=$m polybar --reload mainbar-i3-extra -c ~/.config/polybar/config.ini.ini &
    #   done
    # else
    # polybar --reload mainbar-i3-extra -c ~/.config/polybar/config.ini.ini &
    # fi
    ;;
